...
d3.json("json/states.json", function(error, us) {
  ...
  d3.csv("csv/crime.csv",function(data){
    var first = data[0];
    // get columns
...
function createLegend(){
  ...
  legend.append("text").text("Crime Rate").attr("x",15).attr("y",33);

  var sizes = [ 100,500, 1000, 1500, 2000 ];
  for ( var i in sizes ){
    ...
    legend.append("text")
      ...
      .attr( "x", 100 + circleSize( sizes[sizes.length-1] ) )
...
function setProbeContent(d){
  ...
  var html = "<strong>" + d.CITY + "</strong><br/>" +
            format( Math.abs( val ) ) + " Crime Rate "+ "<br/>" +
            "<span>" + " " + m_y[1] + "</span>";
  probe
    .html( html );
}
